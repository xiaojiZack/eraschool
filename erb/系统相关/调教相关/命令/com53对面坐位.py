...
def com53(active,passive):
    ...
    if check_doing_list(active,passive,53) and check_result!=False:
        ...
        am['快C'] += ((1+passive['开发']['V名器度'])*300)*(1+passive['开发']['腰技']*1)
        ...
        if  obey_check(32,active,passive,com_trait):
            #此处可能需要处理替换的问题
            active['标志']['阴茎占用'] = 53
            append_doing_list(active,passive,53)
...
def undocom53(active,passive):
    a.tmp()['执行列表'].remove([active['CharacterId'],passive['CharacterId'],53])
