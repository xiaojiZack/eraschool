...
comid = 50
def com50(active,passive):

    aname = active['名字']
    pname = passive['名字']
    com_trait = ['C','V','处女破坏','中出','精液']
    am = active['待处理记忆']
    ae = active['待处理经验']
    pm = passive['待处理记忆']
    pe = passive['待处理经验']
    ...
    if check_doing_list(active,passive,50) and check_result!=False:
        ...
        pm['快C'] += 25 * (1+active['开发']['腰技']*1)
        pm['快V'] += 50 * (1+active['开发']['腰技']*1)
        pm['羞耻'] += 200 * (1+active['开发']['腰技']*1)
        pm['欲情'] += 250 * (1+active['开发']['腰技']*1)
        pm['恭顺'] += 300 * (1+active['开发']['腰技']*1)
        pm['恐惧'] += 50 * (1+active['开发']['腰技']*1)
        pm['习得'] += 50 * (1+active['开发']['腰技']*1)
        pm['屈服'] += 200 * (1+active['开发']['腰技']*1)
        pm['反感'] += 10
        ...
        am['习得'] += 50*(1+passive['开发']['腰技']*1)
        am['主导'] += 500*(1+passive['开发']['腰技']*1)
        ae['腰技经验'] += 1
        pe['腰技经验'] += 1
        ae['V插入经验'] += 1
        ...
        if  obey_check(obey_challenge,active,passive,com_trait):
            #此处可能需要处理替换的问题
            active['标志']['阴茎占用'] = 50
            append_doing_list(active,passive,50)
...
def undocom50(active,passive):
    a.tmp()['执行列表'].remove([active['CharacterId'],passive['CharacterId'],50])
