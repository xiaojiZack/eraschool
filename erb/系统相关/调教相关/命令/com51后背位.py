...
def com51(active,passive):
    ...
    if check_doing_list(active,passive,51) and check_result!=False:
        insert(active,passive,'P','V',check_result)
                                
        pm['快V'] += 10 * (1+active['开发']['腰技']*1)
        ...
        if  obey_check(30,active,passive,com_trait):
            #此处可能需要处理替换的问题
            active['标志']['阴茎占用'] = 51
            append_doing_list(active,passive,51)
...
def undocom51(active,passive):
    a.tmp()['执行列表'].remove([active['CharacterId'],passive['CharacterId'],51])
    active['标志']['阴茎占用'] = 0
