...
def com57(active,passive):
    ...
    if check_doing_list(active,passive,57) and check_result!=False:
        ...
        if  obey_check(35,active,passive,com_trait):
            #此处可能需要处理替换的问题
            active['标志']['阴茎占用'] = 57
            append_doing_list(active,passive,57)
...
def undocom57(active,passive):
    a.tmp()['执行列表'].remove([active['CharacterId'],passive['CharacterId'],57])
