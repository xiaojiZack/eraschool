...
def com25(active,passive):
    ...
    if not check_equipment(passive,25):
        a.tmp()['暂停'] = True
        select_size(passive)
        while a.tmp()['暂停']: time.sleep(0.1)
        size = a.tmp()['按摩棒选择']
    else:
        size = passive['身体信息']['阴道']['内容固体']['按摩棒']
    ...
    oil_flag = is_enough_oiling(passive,'V',oilneed)
    if oil_flag >0:
        com_trait.append('润滑不足')
    
    if (size_flag or a.tmp()['调教数据']['尺寸警告标志'] == False):
        if check_equipment(passive,25):
            if oil_flag >0:
